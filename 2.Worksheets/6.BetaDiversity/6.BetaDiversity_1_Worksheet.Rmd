...
> ***Answer 2c***:  Fish richness does not allow us to make predictions regarding abundance since we see less fish richness in the upstream area but more > abundance of the Brown trout. 
...
beta.w(doubs$fish) # Local richness 2.16
beta.w(doubs$fish, pairwise = TRUE)
beta.w(doubs$fish)
...
> ***Answer 3a***:  The local richness of the fish in the Doubs river is 2.16. 
> ***Answer 3b***:  0.714? <INSERT ANSWERS TO ALL OF THESE!!!!!!!!!!!!> 
> ***Answer 3c***:  Additively changes the B meaning by how many times more diverse the region is than local sites to how many more species exist in the regional pool than in local sites. 
...
> ***Answer 5a***:  'fish.db' represents dissimilarity not only because we used the Bray-Curtis metric to calculate the matrix (which is a measure of dissimilarity ) but also because the matrix is triangular, the diagonal of comparing each site to itself is missing as a default 
> characteristic. 
> ***Answer 5b***: <INSERT ANSWER HERE!!!!!!!!!!!> 
...
# Use this info to calculate and add species scores
...
# Added from Canan's slack message since the add.spec.scores isn't working # START Here! 
