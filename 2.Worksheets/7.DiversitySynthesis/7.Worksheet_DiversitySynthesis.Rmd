...
# Subset the data 
gummy_sbys <- (gummy, grepl(("Community")))???
gummy_subset <- subset.matrix(gummy, 
...
# calculate observed richness of community
# gummy.S <- S.obs(gummy)

# Idenfify sample with fewest occupancies 
min.N <- min(rowSums(gummy))

# Rarifying to rarefy each sample to this level
S.rarefy <- rarefy(x = gummy, sample = min.N, se = TRUE)
...
```{r}
library(ggplot2)
# Plotting errors of Shannon entropy for source communities 
